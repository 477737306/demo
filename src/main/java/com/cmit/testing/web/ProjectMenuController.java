...
public class ProjectMenuController extends BaseController {
    ...
    public JsonResultUtil copy(@RequestBody Map<String ,Object> map){
        ...
        for (int i =0 ;i < list.size() ;i++) {
            if(!projectMenuService.checkStatus(Integer.parseInt(list.get(i).get("id")))){
...
//
    ...
    public JsonResultUtil deleteByIds(@RequestBody Map<String ,Object> map){
        ...
        for (Integer id : ids) {
            if(!projectMenuService.checkStatus(id)){
                throw new TestSystemException("项目已被归档，不可操作！");
            }
        }
...
//        noType.add("scriptsData");
    ...
    @ResponseBody
    @Permission
    public JsonResultUtil updateSatus(@PathVariable("id") Integer id ,@RequestBody Map<String,String> map){
        String type = map.get("type");
        projectMenuService.updateStatus(id,type);
        return new JsonResultUtil(200,"操作成功");
    }
}
