...
/**
 * @author XieZuLiang
 * @description TODO 字符串相关的常用方法
 * @date 2018/8/30 0030 上午 9:53.
 */
public class StringUtils {
    ...
    /**
	 * 校验输入的字符串是否是键盘上的连续三个字符,是返回true,否返回false
	 * 
	 * @param str
	 * @return
	 */
	public static boolean isOrderKeyCode(String str) {
		char[][] keyCode = { { '1', '2', '3', '4', '5', '6', '7', '8', '9', '0', '-', '=' },
				{ 'q', 'w', 'e', 'r', 't', 'y', 'u', 'i', 'o', 'p', '[', ']' },
				{ 'a', 's', 'd', 'f', 'g', 'h', 'j', 'k', 'l', ';', '\'', '\\' },
				{ 'z', 'x', 'c', 'v', 'b', 'n', 'm', ',', '.', '/' } };
		char[][] keyCode2 = { { '!', '@', '#', '$', '%', '^', '&', '*', '(', ')', '_', '+' },
				...
				{ 'z', 'x', 'c', 'v', 'b', 'n', 'm', '<', '>', '?' } };
		boolean flag = false;
		if (isOrderKeyCodeImpl(keyCode, str) || isOrderKeyCodeImpl(keyCode2, str)) {
			flag = true;
		}
		return flag;
	}
	 ...
	 * @return
	 */
	public static boolean isOrderKeyCodeImpl(char[][] keyCode, String str) {
		char[] c = str.toLowerCase().toCharArray();
		List<Integer> x = new ArrayList<Integer>();
		List<Integer> y = new ArrayList<Integer>();
		// 将字符串每个字符在数组中的位置，写入到集合中
		...
		for (int i = 0; i < x.size() - 2; i++) {
			int x1 = x.get(i), x2 = x.get(i + 1), x3 = x.get(i + 2);
			int y1 = y.get(i), y2 = y.get(i + 1), y3 = y.get(i + 2);
			if (x1 == x2 && x2 == x3) {// 三者在同一行上
				if (y1 > y3) {// 如果键盘按键是从右往左
					if (y1 - 1 == y2 && y1 - 2 == y3) {
						flag = true;
						break;
					}
			...
			} else if (x1 != x2 && x2 != x3 && x1 != x3) {// 三者均不在同一行上
				if (x1 >= x3) {// 如果键盘按键是从下到上
					if (y1 > y3 || (y1 == y2 && y1 == y3)) {// 如果键盘按键是从右往左
						if ((y1 == y2 && y1 == y3) || (y1 - 1 == y2 && y1 - 2 == y3)) {
							flag = true;
							break;
						}
					} else {// 如果键盘按键是左往右
	...
	/**
	 * 是否包含3位及以上的正序及逆序连续字符,键盘123456789数字对应的正序逆序特殊字符,是返回true,否返回false
	 * 
	 * @param string
	 * @return
	 */
	public static boolean isOrderChar(String string) {
		char[] cc = string.toCharArray();
		int length = cc.length;
		if (length <= 2) {
			return false;
		} else if (length == 3) {
			if (isOrderCharImpl(string)) {
				return true;
			} else {
				return false;
			}
	...
	public static boolean isOrderCharImpl(String string) {
		char[] cc = string.toCharArray();
		return Math.abs(cc[0] - cc[1]) == 1 && (cc[0] - cc[1]) == (cc[1] - cc[2]);
	}
